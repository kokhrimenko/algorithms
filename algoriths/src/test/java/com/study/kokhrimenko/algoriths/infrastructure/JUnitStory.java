...
public class JUnitStory<T extends ReflectionCreatedClass<T>> {
	...
	public JUnitStory(Class<?> testClass, T testDSFactory) {
		super();
		this.testClass = testClass;
		this.logger = LoggerFactory.getLogger(testClass);
		try (BufferedReader br = new BufferedReader(new InputStreamReader(testClass.getResourceAsStream(String.format("%s.%s", testClass.getSimpleName(), DS_FILENAME_EXTENSION))))) {
			String line = "";
			while ((line = br.readLine()) != null) {
				String[] items = line.split(ITEMS_DELIMITER);
				if (items == null || items.length != testDSFactory.getAllowedCountOfConstructorArguments()) {
					throw new IllegalArgumentException("File with test data contains some wrong data: " + line);
				}
				
				testedDataSet.add(testDSFactory.instanciate(items));
			}
		} catch (IOException e) {
